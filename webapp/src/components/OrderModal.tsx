...
export function OrderModal({
  ...
  const [step, setStep] = useState<"cart" | "info" | "confirm" | "sent">("cart");
...
${latitude && longitude ? `📌 *Mapa:* https://www.google.com/maps?q=${latitude},${longitude}` : ""}
${notes ? `📝 *Notas:* ${notes}` : ""}
━━━━━━━━━━━━━━━━━━
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-2xl sm:rounded-2xl bg-surface shadow-2xl animate-scale-in">
        ...
        <div className="p-5">
            ...
            <>
              <div className="space-y-3 mb-6">
                {items.map((ci) => (
                  <div
                    key={ci.item.id}
                    className="flex items-center justify-between gap-3 rounded-xl border border-border/50 bg-surface-alt p-3"
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-text-muted">
                        ${ci.item.price.toLocaleString("es-AR")} c/u
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="min-w-[1.25rem] text-center text-sm font-bold text-text">
                        {ci.quantity}
                      </span>
                      <button
                        onClick={() => onAdd(ci.item.id)}
                        className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary text-white transition-colors"
                      >
                        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                    ...
                    <div className="text-sm font-bold text-text w-20 text-right">
                      ${(ci.item.price * ci.quantity).toLocaleString("es-AR")}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full rounded-xl bg-gradient-to-r from-primary to-amber-500 px-5 py-3 text-sm font-semibold text-white shadow-md shadow-primary/25 hover:shadow-lg hover:shadow-primary/30 hover:-translate-y-0.5 transition-all"
              >
                Continuar
              </button>
            </>
            ...
            <>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 rounded-xl bg-gradient-to-r from-primary to-amber-500 px-5 py-3 text-sm font-semibold text-white shadow-md shadow-primary/25 hover:shadow-lg hover:shadow-primary/30 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-md"
                >
                  Revisar Pedido
                </button>
              </div>
          ...
          {step === "confirm" && (
            <>
              <div className="rounded-xl border border-border/50 bg-surface-alt p-4 mb-4">
                ...
                <div className="mt-3 border-t border-border/50 pt-3 flex justify-between">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={sending}
                  className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-[#25D366] px-4 py-3.5 text-sm font-bold text-white shadow-md shadow-green-500/20 hover:shadow-lg hover:shadow-green-500/30 hover:-translate-y-0.5 transition-all disabled:opacity-50"
                >
                  {sending ? (
          ...
          {/* Step 4: Order sent confirmation */}
          {step === "sent" && (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-text-secondary leading-relaxed mb-6">
                Tu pedido fue registrado y enviado por WhatsApp a{" "}
                <strong>{restaurantName}</strong>. Guardá tu número de pedido
