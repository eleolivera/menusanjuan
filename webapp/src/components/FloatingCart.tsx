...
export function FloatingCart({
  ...
  onViewOrder?: () => void;
  /** Populated when the customer is signed in + has an active Redemption
   *  ready to auto-apply. Shows a 🎁 pill above the cart so they know the
   *  free item is coming without having to submit first. */
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-1.5 animate-scale-in">
      ...
      <button
        onClick={onClick}
        className="flex items-center gap-3 rounded-2xl bg-gradient-to-r from-primary to-amber-500 px-5 py-3.5 text-white shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 hover:-translate-y-0.5 transition-all"
      >
